...
// V14: the cinematic loading screen is intentionally preserved but dormant.
  ...
  return (
    <main id="main-content">
      ...
      <nav className="nav shell" aria-label="Main navigation">
        <a className="brand" href="#top" aria-label={`${content.owner.displayName} — portfolio`}>
          <span className="brandMark" aria-hidden="true"><span /></span>
          <span className="brandCopy">
            <strong>{content.owner.username}</strong>
            <small>ROBLOX PORTFOLIO</small>
          </span>
        </a>
      ...
      <div className={`mobileNav ${mobileNavOpen ? "isOpen" : ""}`} id="mobile-navigation" aria-hidden={!mobileNavOpen}>
        ...
        <div className="mobileNavPanel">
          <span className="mobileNavKicker">{content.owner.username}</span>
          <div className="mobileNavLinks">
            ...
            {visibleMedia.length > 0 && <a href="#media" onClick={() => setMobileNavOpen(false)}><span>04</span>{labels.navMedia}</a>}
            {visibleGames.length > 0 && <a href="#games" onClick={() => setMobileNavOpen(false)}><span>05</span>{labels.navGames}</a>}
            <a href="#reviews" onClick={() => setMobileNavOpen(false)}><span>06</span>{labels.navReviews}</a>
            <a href="#contact" onClick={() => setMobileNavOpen(false)}><span>07</span>{labels.navContact}</a>
          </div>
      ...
      <section className="hero shell" id="top">
        <div className="heroGrid">
          <div>
            <div className="heroMeta" aria-label="Portfolio specialties">
              <span>{labels.studioBadge}</span>
              <span>{labels.roleBadge}</span>
            </div>
            <h1>
