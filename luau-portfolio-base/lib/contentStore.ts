...
export class PortfolioContentLoadError extends Error {
  constructor(message = "Could not load portfolio content") {
    super(message);
    this.name = "PortfolioContentLoadError";
  }
}
...
export async function getPortfolioContentState(): Promise<PortfolioContentState> {
  const supabase = getAdminClient();

  // Never silently fall back to the template when the database is unavailable.
  // A temporary Supabase/configuration failure should be visible to the caller so
  // the public client can retry or use a verified last known-good snapshot.
  if (!supabase) {
    throw new PortfolioContentLoadError("Supabase is not configured");
  }
  ...
  if (error) {
    console.error(error);
    throw new PortfolioContentLoadError(error.message);
  }

  // No row really means the bundled template is the intended state. We expose
  // that fact to the client so it never mistakes a template response for the
  // owner's saved portfolio and caches it as a backup.
  if (!data?.content) {
    return { content: defaultPortfolio, source: "template", updatedAt: null };
  }

  return {
    content: upgradeContent(data.content as Partial<PortfolioContent>),
    source: "saved",
...
export async function getPortfolioContent(): Promise<PortfolioContent> {
  return (await getPortfolioContentState()).content;
}
