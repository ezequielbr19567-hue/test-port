...
export async function POST(request: NextRequest) {
  const rate = consumeRateLimit(requestFingerprint(request, "public-review"), 4, 60 * 60 * 1000);
  if (!rate.allowed) {
  ...
  if (!displayName || !title || !description || !(image instanceof File) || image.size === 0) {
  ...
  if (!REVIEW_IMAGE_TYPES.has(image.type)) {
  ...
  try {
    uploaded = await uploadPublicMedia(image, "reviews");
